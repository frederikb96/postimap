...
### Fixed
- `supersede()` only checked that `replaces_message_id` named a message in the same account, so any message could be named for expunge through the outbox insert -- an API with no authentication of its own. Sending or saving a draft could delete any message in the account, not only the draft it was meant to replace. The check now requires the named message to actually be a draft (`is_draft`, or its folder's `special_use = 'drafts'`); anything else is ignored
- A send's destination folder (Sent or Drafts) was resolved after the SMTP delivery, so a folder lookup failure dead-lettered an entry whose mail had already reached the recipient -- reported as a failed send, inviting a resend that would duplicate it. The folder is now resolved first; an entry whose SMTP delivery already succeeded on an earlier attempt is marked sent, with a distinct `sent_copy` notification, rather than dead, when the folder still can't be found
- A full resync excluded expunged rows and rows with a NULL `imap_uid` from a folder's known-UID set, but a row is in either state precisely because an outbound delete or move is still queued for it -- the server hasn't been told yet, so the message is still there under its old UID. The remote UID then came back as unmatched: a pending delete's target was reinserted with `expunged_at` cleared, undoing the app's write, and a pending move's source was inserted a second time under the folder it had already left in PG. Queued UIDs now stay in the known set for as long as their `sync_queue` entry is unresolved
- Account start ran every folder through a backfill sync unconditionally, including folders that had already completed their initial sync. Backfill suppresses per-message insert events, and an already-synced folder never fires `sync_complete` either -- so a restart silently mirrored whatever mail had arrived while the service was down, with no event reaching a listening consumer. Backfill now follows the same completed-first-sync check the periodic cycle already uses
- Rows a single consumer transaction enqueues share one `created_at` -- `now()` inside a transaction never advances -- so ordering by `created_at` alone left the claim order, and the queue's own coalescing, unspecified for those rows. A move chain enqueued that way could coalesce into a move whose source was the wrong hop, with nothing to act on. Both the claiming query and the coalescing sort now break the tie on `id`
- `reportAbandonedIdle` wrote its `sync_notifications` row outside the sync-writer transaction, so the resulting event carried `origin: "app"` -- indistinguishable from a consumer's own write, for something PostIMAP reported about itself
...
- `sync_notifications`: a durable, acknowledgeable record of a write that never reached the server. `sync_error` on `postimap_events` announced the moment it happened and nothing survived it, so a client that was not listening never learned, there was no way to ask what had gone wrong lately, and nothing recorded that a user had seen one. A row is written once per operation that gives up permanently -- including a send that never left -- carrying the server's error in full plus the subject and `Message-ID` captured at the time, so it still renders after retention removes the message it names. `acknowledged_at` is the only consumer-writable column; the insert fires a `notification` event so a consumer listens rather than polls. Retention is keyed on `acknowledged_at`, never `created_at`: a notification nobody has seen is never removed by age
- A permanently failed write now has the mirror put right within a second instead of at the next poll, and only for the message it concerns. A general resync would not do it -- the server never saw the write, so on a CONDSTORE or QRESYNC server it has no change to report and the divergence would survive every later cycle. The specific operation is undone instead: a failed flag change re-reads that message's flags from the server, a failed move restores `folder_id` and `imap_uid`, a failed delete clears `expunged_at`. `reverted_at` records that it happened

- IMAP push is chosen per folder at runtime instead of per deployment. `folders.idle_requested` is consumer-writable and `folders.idle_status` reports what became of the request -- `off`, `watching`, `unsupported`, or `failed`. `sync.idle_folders` still supplies the default, but it seeds a folder's preference exactly once, so switching every folder off stays off rather than being re-seeded from config on the next start. The watcher adds and drops folders while running, so a consumer's write takes effect on the next sync cycle rather than needing a restart
- A watch that exhausts its reconnect attempts writes `idle_status = 'failed'` and a `sync_notifications` row. Reconnection already backed off and retried, but when it finally stopped nothing in the database changed and nothing fired -- the folder quietly ceased to be real-time while still syncing on the interval, so nothing looked broken and nobody was told

### Changed
- A full sync fetches only the messages PG is missing, instead of re-downloading every body in the folder. `runStart` runs one for every folder on every account start, so a restart cost as much as the mailbox is big -- measured at 25s against 51ms for the incremental path on a 2000-message folder where nothing had changed, on every deploy, every crash and every rolling update. The exhaustive re-walk was also the only thing that would ever notice a UID missing from the *middle* of a folder, since the incremental path only looks forward from the last known UIDNEXT, so the diff now runs in both directions: fetch what IMAP has and PG lacks, expunge what PG has and IMAP lacks. An already-mirrored folder costs one `SEARCH` and no bodies; a folder with a hole is still repaired. On a provider that caps daily download volume this is also the difference between a large first sync taking several days and never finishing at all, since each restart previously re-pulled what the interrupted run had already fetched
- Change detection reads only the mirrored state its tier will consult. Every cycle previously loaded every non-expunged message of the folder with seven columns and built a `Set` of flags per row, before the tier was even chosen -- and only the full-diff tier ever reads that map. On a QRESYNC or CONDSTORE server it was built and discarded once a minute per folder, which made the fast path cost as much as the folder is big exactly where the design promises it costs as much as *changed*. The QRESYNC path driven by the SELECT's own VANISHED/FETCH responses now asks for membership of the UIDs actually in play rather than the whole folder
- On servers with neither CONDSTORE nor QRESYNC, a cycle whose mailbox message count and UIDNEXT have not moved since the last reconciliation skips the UID `SEARCH` and the fetch of every flag set. Both numbers come free from the `SELECT` already held. A flag changed by another client moves neither, so the skip expires: after `sync.full_tier_max_skip_seconds` the full diff runs regardless
- `sync_state` progress is written per folder rather than once after the whole folder loop. An initial backfill can run for hours, and it is the SQL-native way to watch one -- updated only at the end, it showed nothing moving for the entire run
- A burst of IMAP IDLE notifications produces one settled sync rather than one per event. Another client marking fifty messages read arrives as fifty separate notifications; each queued its own full change-detection pass

### Fixed
- `sync_notifications` inserts reported `origin: "app"`. Both call sites wrote outside a `withSyncWriter` transaction, so the event trigger read an unset writer GUC and named the consumer as the author of PostIMAP giving up on its own work -- the same defect fixed for `sync_error` in v1.2.1, reintroduced on its sibling event. Nothing but PostIMAP ever inserts into that table; there is no consumer INSERT grant
- A move that dead-lettered because the message was in neither folder wrote the pre-move UID back and set `reverted_at`, claiming the mirror had been checked against a position it had just disproven -- pointing the row at a UID naming nothing, which every later operation would then resolve against. The revert now verifies the message is still where it is about to say it is, and leaves `reverted_at` unset when it is not. A move that failed outright, where the message never left, still reverts as before
- One IDLE connection failure could start several reconnect loops. A failed connect rejects its promise *and* emits `close` a tick later, and both paths triggered a reconnect -- two loops racing over the same client field, each able to destroy the connection the other had just established, each counting its own retries and reporting giving up separately. One folder could hold several connections against a budget the whole feature is built around being small
- On a server without IDLE, `sync.idle_folders` was silently ignored forever and every folder reported `idle_status = 'off'` rather than `'unsupported'` -- indistinguishable from a consumer who had turned everything off. Seeding and status now run whether or not a watcher exists
- `010_folder_crud.down()` failed on any database where a folder operation had ever been queued: it restored the narrower `sync_queue_action_check` while rows naming `folder_create` were still present, and PostgreSQL refuses a CHECK the existing data violates. Only a rollback path, which is exactly when a second failure is least welcome
