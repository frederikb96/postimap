...
- **Folder list**: reconciled from the server's `LIST` on every sync cycle, so a folder created or removed in another mail client shows up without restarting anything. A server advertising `OBJECTID` also gets rename detection via the stable `MAILBOXID`.
